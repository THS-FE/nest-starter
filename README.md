...
├─src 源码目录
│  ├─filters 过滤器目录，实现系统内建的异常过滤器，自定义异常处理，将异常更友好地响应给前端
│  ├─interceptors 拦截器目录，实现在客户端与服务器请求或者响应前，拦截添加一道逻辑处理
│  ├─interfaces 接口目录，约束传入对象类型
│  ├─middlewares 中间件目录，实现匹配路由之前或者匹配路由完成做的一系列的操作。
│  ├─modules 业务模块
│  │  ├─auth 认证模块，内置JWT，实现了token认证，需要接口认证的模块需要导入该模块
│  │  │  └─dtos 数据传输对象存放目录，接口参数验证及在线接口文档定义需要在此完成
│  │  ├─comm 公共模块，封装一些通用的接口，比如附件的上传，下载功能
│  │  │  ├─dtos 数据传输对象存放目录，接口参数验证及在线接口文档定义需要在此完成
│  │  │  └─services 服务，存放业务逻辑或者数据处理等操作代码
│  │  │      ├─comm-utils 工具类，逐步完善
│  │  │      ├─file-utils 文件处理
│  │  │      └─http-client 网络请求客户端
